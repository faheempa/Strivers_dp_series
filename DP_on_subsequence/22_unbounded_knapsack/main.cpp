// Question : https://www.codingninjas.com/studio/problems/unbounded-knapsack_1215029
...
int solve(vector<int> &wt, vector<int> &val, int index, int remainingWt)
{
    ...
    if (wt[index] <= remainingWt)
        take = val[index] + solve(wt, val, index, remainingWt - wt[index]);
    // get max
...
int memoization(vector<int> &wt, vector<int> &val, int index, int remainingWt, vector<vector<int>> &dp)
{
    ...
    int take{0};
    if (wt[index] <= remainingWt)
        take = val[index] + memoization(wt, val, index, remainingWt - wt[index], dp);
    // get max
...
int tabulation(vector<int> &wts, vector<int> &val, int n, int maxWt)
{
    ...
    for (int index = 1; index < n; index++)
    {
        for (int wt = 0; wt <= maxWt; wt++)
        {
            ...
            int take{0};
            if (wts[index] <= wt)
                take = val[index] + dp[index][wt - wts[index]];
            // get max
